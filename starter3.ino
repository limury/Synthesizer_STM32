...
namespace DMASync {
    ...
    TaskHandle_t xDMASyncHandle = NULL;
    ...
    void sampleGeneratorTask( void* pvParameters ){
        ...
        while(1){
            //generate 100 samples in DMASync::NextDMABuffer
            // DMASync::NextDMABuffer uses 12 bits.
            for (uint8_t i = 0; i < HALF_BUFFER_SIZE; i++){
                phaseAcc += increment;
                // DMASync::NextDMABuffer[i] = (phaseAcc >> 24) >> (8 - knob_3_pos/2);
                // digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
                DMASync::DMACurrBuffPtr[i] = (phaseAcc >> 20) >> (8 - knob_3_pos/2);
            }

            if (DMASync::DMATMP == DMASync::DMACurrBuffPtr){
                Serial.println("Jerrror");
                digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
            }
            DMASync::DMATMP = DMASync::DMACurrBuffPtr;
            // memcpy( (void*) DMASync::DMACurrBuffPtr, (void*) DMASync::NextDMABuffer, HALF_BUFFER_SIZE*4 );
            ...
            // vTaskSuspend( DMASync::xDMASyncHandle );
...
volatile uint8_t tmp = 0;
    void DMA_Buffer_End_Callback( DMA_HandleTypeDef* hdma ){
        __atomic_store_n( &DMASync::DMACurrBuffPtr, (uint32_t*) DMASync::DMAHalfBuffPtr, __ATOMIC_RELAXED );

        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        // vTaskNotifyFromISR( DMASync::xDMASyncHandle, &xHigherPriorityTaskWoken );

        // works
        uint32_t ulStatusRegister;
        xTaskNotifyFromISR( DMASync::xDMASyncHandle, 0, eNoAction, &xHigherPriorityTaskWoken );
        portYIELD_FROM_ISR( xHigherPriorityTaskWoken );
        
        // Serial.println("a");
        // set next data transfer to go to half buffer
        // vTaskNotifyGiveFromISR( DMASync::xDMASyncHandle, &xHigherPriorityTaskWoken );
            // tmp_var++;
            // if (tmp_var == 440)
            //   Serial.println("10 seconds");

        // if ( xTaskResumeFromISR( DMASync::xDMASyncHandle ) == pdTRUE){
        //     portYIELD_FROM_ISR( xHigherPriorityTaskWoken );
        // }
        // else {
        //     digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
    ...
    void DMA_Buffer_Half_Callback( DMA_HandleTypeDef* hdma ){
        __atomic_store_n( &DMASync::DMACurrBuffPtr, (uint32_t*) DMASync::DMABuffer, __ATOMIC_RELAXED );
        ...
        // digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
        // // Allow S
        // xSemaphoreGiveFromISR( DMASync::xDMASyncHandle, &xHigherPriorityTaskWoken );
        // if (xSemaphoreGiveFromISR( DMASync::xDMASyncHandle, NULL ) == errQUEUE_FULL){
...
void setup() {
    ...
    DMASync::DMABuffer = (uint32_t*) malloc(BUFFER_SIZE*sizeof(uint32_t));
    DMASync::DMAHalfBuffPtr = &DMASync::DMABuffer[HALF_BUFFER_SIZE];
    DMASync::DMACurrBuffPtr = DMASync::DMABuffer;
    DMASync::NextDMABuffer = (uint32_t*) malloc(HALF_BUFFER_SIZE*sizeof(uint32_t)); 
    DMASync::DMATMP = DMASync::DMAHalfBuffPtr;
    // for (uint8_t i = 0; i < BUFFER_SIZE; i++){
    //     DMASync::DMABuffer[i] = 0;
    // }

    // uint16_t phaseAcc = 0;
    // uint16_t increment = 1310;
    // for (uint8_t i = 0; i < BUFFER_SIZE; i++){
    //     DMASync::DMABuffer[i] = 0;//phaseAcc >> 4;
    //     phaseAcc += increment;
    // }
    //Set pin directions
    ...
    xTaskCreate( DMASync::sampleGeneratorTask, "sampleGenerator", 256, NULL, 9, &DMASync::xDMASyncHandle );

    HAL_TIM_Base_Start( &htim2 );


    HAL_DAC_Start_DMA( &hdac1, DAC1_CHANNEL_1, DMASync::DMABuffer, BUFFER_SIZE, DAC_ALIGN_12B_R );
    ...
    // // sampleTimer->resume();
    HAL_DMA_RegisterCallback( &hdma_dac_ch1, HAL_DMA_XFER_CPLT_CB_ID, &DMA_Buffer_End_Callback);
    HAL_DMA_RegisterCallback( &hdma_dac_ch1, HAL_DMA_XFER_HALFCPLT_CB_ID, &DMA_Buffer_Half_Callback);
    hdma_dac_ch1.XferCpltCallback     = DMA_Buffer_End_Callback;
    hdma_dac_ch1.XferHalfCpltCallback = DMA_Buffer_Half_Callback;
